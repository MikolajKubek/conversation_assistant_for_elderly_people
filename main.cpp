...
std::string loop_callback(std::string user_prompt, std::shared_ptr<GptInterface> llm_interface, std::shared_ptr<ApiManager> api_manager) {
    ...
    while (!is_final && interaction_counter < MAX_INTERACTOINS) {
        interaction_counter++;
        user_prompt_wrapped = prompt_processor::render_followup_template(user_prompt, processed_response.first, available_apis);
...
int main() {
    ...
    std::unique_ptr<AssistantApi> weather_api = std::make_unique<WeatherApi>(openweather_key);
